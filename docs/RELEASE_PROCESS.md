...
the script bundle and every newly introduced candidate module definition. Existing
mutable canonical module definitions on `main` and generated rules are protected
separately by repository invariants, source hashes, generated byte comparison,
and live profile validation.
...
- feature branches: development only. Do not reference a feature branch from a
  public module.
- `rewrite/Surge/candidates/`: non-public definitions for script-backed modules
  that do not exist in the active distribution yet. They may pin the single
  candidate tag, but must not be advertised as stable install URLs or moved to
  canonical top-level paths before activation.
...
5. Add `releases/<tag>.json` with `integrity: intact`, `distribution: candidate`,
   ...
   When the payload introduces a new script-backed module, place its definition
   under `rewrite/Surge/candidates/` and pin that staged definition to the
   candidate tag. Add its repository path and SHA-256 to the manifest `modules`
   array. The verifier treats discovered script modules as a closed set and
   reconstructs both script and candidate-module bytes from `release_commit`.
   The staged raw `main` URL is not an install entry and remains unusable until
   registration and tag creation complete.
...
11. Verify every tagged script and candidate-module URL returns HTTP 200 with
...
12. Create a temporary local copy of each affected existing module and replace only its
    immutable `script-path` tag with the candidate tag. Install those test-only
    module definitions on the explicitly selected Surge iOS device without
    publishing them under canonical `main` URLs. Do not install or enable them in
    Surge Mac. Confirm the candidate tag and script paths before
    ...
    A newly introduced module must use its manifest-pinned
    `rewrite/Surge/candidates/` definition from the immutable candidate tag after
    that tag exists; it is still an iOS-only test entry and must retain its
...
15. For a passed candidate, open an activation pull request that changes the old
    ...
    Move each passed new module from `rewrite/Surge/candidates/` to its canonical
    top-level path, remove the candidate suffix from its display name, update the
    verifier module mapping, and add its permanent `main` install URL in this
    activation change. Before activation, separately revoke every rollback target
    whose script inventory does not contain the future active inventory.
